...
func ImportReference(filepath, importCode, structName, packageName, groupName string) error {
	...
	parseFile, err := parser.ParseFile(fileSet, filepath, nil, parser.ParseComments)
	if err != nil {
		return err
	}
	trimSpace := strings.TrimSpace(importCode)
	visitor := &Visitor{
	...
	if trimSpace == "" {
		ast.Print(fileSet, parseFile)
	}

	ast.Walk(visitor, parseFile)

	var outPut []byte
	buffer := bytes.NewBuffer(outPut)
	err = format.Node(buffer, fileSet, parseFile)
	if err != nil {
		log.Fatal(err)
	}
